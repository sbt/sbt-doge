...
`CrossPerProjectPlugin` overrides sbt's `+` and `++` commands and uses doge's implementation that aggregates command respecting `crossScalaVersions` at each subproject. It is not an auto-plugin and so must be enabled using `enablePlugins(CrossPerProjectPlugin)`.
